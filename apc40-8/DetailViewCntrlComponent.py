...
# File: DetailViewCntrlComponent.pyc (Python 2.5)
...
class DetailViewCntrlComponent(ControlSurfaceComponent):
    ...
    def set_device_nav_buttons(self, left_button, right_button):
        if not left_button == None and isinstance(left_button, ButtonElement):
            raise AssertionError
        if not right_button == None and isinstance(right_button, ButtonElement):
            raise AssertionError
    ...
    def _nav_value(self, value, sender):
        if not sender != None or sender in (self._left_button, self._right_button):
            raise AssertionError
        if self.is_enabled() and not (self._shift_pressed):
